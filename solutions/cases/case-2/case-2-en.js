(() => {
  ...
  function resetPdfCase() {
    ...
    pdfArchiveFiles.innerHTML =
      '<div class="drive-empty">Archive is empty</div>';
    ...
    pdfLog.textContent =
      "Ready to start. The PDF_IN folder contains 3 demo PDF files.";
  ...
  async function runPdfCase() {
    ...
    for (
      ...
      pdfLog.textContent =
        `${current.name} found in the PDF_IN folder. Reading the file...`;
      ...
      pdfLog.textContent =
        current.type === "invoice"
          ? "Invoice detected. Parsing document number, date, article, position, quantity, price and tariff code."
          : "Packing List detected. Parsing weight, Net Weight, Gross Weight and packaging details.";
      ...
      pdfLog.textContent =
        current.type === "invoice"
          ? `Data written to the RAW sheet. ${current.rows.length} line items recognized.`
          : "Data written to the PACKING sheet. Weight data is ready to be matched with RAW.";
