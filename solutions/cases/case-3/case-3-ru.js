(() => {
  ...
  const products = [
    ...
    ["Монитор LG 24", "Мониторы", 9000],
    ["Монитор Samsung 27", "Мониторы", 11800],
    ["Мышь Logitech M185", "Аксессуары", 450],
    ["Клавиатура A4Tech", "Аксессуары", 750],
    ["SSD Kingston 1TB", "Комплектующие", 3200],
    ["Роутер TP-Link", "Сетевое оборудование", 2100],
    ["Принтер Canon", "Оргтехника", 7600]
  ];
    ...
    rawBody.innerHTML =
      data.map(row => `
        <tr>
          ...
          <td>${row.year}</td>
          <td>${row.date.toLocaleDateString("ru-RU", { month: "long" })}</td>
          <td>${row.day}</td>
  ...
  function renderMonthlyChart(rows) {
    ...
    if (
      ...
      peakNode.textContent =
        `${monthNames[peakIndex]} · ${formatNumber(values[peakIndex])} грн`;
    } else {
      peakNode.textContent = "—";
    }
  }
        ...
        ([product, item]) => {
          ...
          return `
            <tr>
              ...
              <td>${formatNumber(item.amount)} грн</td>
  ...
  function renderDashboard() {
    ...
    status.textContent =
      `${rows.length} из ${data.length} строк в текущем срезе`;
