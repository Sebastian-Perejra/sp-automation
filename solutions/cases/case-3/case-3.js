(() => {
  ...
  function renderRawRows() {
    rawBody.innerHTML =
      data.map(row => `
        <tr>
          <td>${row.date.toLocaleDateString("uk-UA")}</td>
          <td>${row.year}</td>
          <td>${row.date.toLocaleDateString("uk-UA", { month: "long" })}</td>
          <td>${row.day}</td>
  ...
  function renderDashboard() {
    ...
    status.textContent =
      `${rows.length} з ${data.length} рядків у поточному зрізі`;
